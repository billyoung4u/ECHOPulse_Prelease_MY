...
#此文件定义了 ST-MEM 模型架构。它通过“掩码重建”的方式（类似 BERT），
# 从未标注的大量 ECG 数据中学习心电图的特征。

from .encoder.st_mem_vit import ST_MEM_ViT, TransformerBlock
"""
该文件定义了 完整的ST-MEM 模型架构。包括编码器（直接调用st_mem_vit.py）和解码器
它通过“掩码重建”的方式（类似 BERT），从未标注的大量 ECG 数据中学习心电图的特征。
核心组件包括编码器和解码器，编码器基于 Vision Transformer (ViT) 结构，
将输入的多导联时间序列划分为补丁并进行嵌入，解码器则尝试重建被掩码的补丁。
模型通过引入导联嵌入和分隔符嵌入来捕捉导联间的关系，同时使用位置嵌入来保留时序信息。
