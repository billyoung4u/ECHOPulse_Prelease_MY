...
"""
该文件仅有stmem模型的的编码器部分（后续st_mem.py会直接复用该文件作为其解码器部分）
用于处理多导联时间序列数据（如心电图）。
模型通过将时间序列划分为补丁，并为每个导联添加独特的嵌入向量，结合位置嵌入和分隔符嵌入来捕捉时序和导联信息。
核心组件包括补丁嵌入层、多头自注意力机制、前馈神经网络以及残差连接，最后通过归一化层输出特征表示。
"""
...
class ST_MEM_ViT(nn.Module):
    def __init__(self,
        ...
        # classifier head @ not using self.head remove later : sk
    ...
    def forward_encoding(self, series):
        ...
        # print ("NICE LETS CHECK EKG ", series.size())
        x = self.to_patch_embedding(series.float())
        b, _, n, _ = x.shape
        ...
        # only utilize second channel. sk 
        x = x[:, 1:2, :, :]
        
        x = torch.mean(x, dim=(1, 2))
        # print ("DONE ST_MEM VIT ", series.size())
        return self.norm(x)
